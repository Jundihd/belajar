...
func main_menu(mahasiswa daftarMahasiswa, matakuliah daftarMatakuliah, nMahasiswa, nMatakuliah int) {
	...
	fmt.Println("3. Menambahkan Matakuliah & Nilai ke Mahasiswa")
	fmt.Println("4. Edit Data Matakuliah Mahasiswa")
	fmt.Println("5. Hapus Data Matakuliah dari Mahasiswa")
...
func menu_data_mahasiswa(mahasiswa daftarMahasiswa, matakuliah daftarMatakuliah, nMahasiswa, nMatakuliah int) {
	...
	fmt.Println("5. Tampil Daftar Mahasiswa secara Terurut NIM Terkecil-Terbesar")
	fmt.Println("6. Tampil Daftar Mahasiswa secara Terurut NIM Terbesar-Terkecil")
	fmt.Println("7. Tampil Daftar Mahasiswa secara Terurut Jumlah SKS Terkecil-Terbesar")
	fmt.Println("8. Tampil Daftar Mahasiswa secara Terurut Jumlah SKS Terbesar-Terkecil")
...
func pilihanEditMahasiswa(P *daftarMahasiswa, index int) {
	for {
		...
		fmt.Println("1. NIM")
		fmt.Println("2. Nama")
		fmt.Println("3. Selesai Mengedit")
...
func binarySearchMatakuliah(B *daftarMatakuliah, N int, kode string) int {
	var left, right int
	left = 0
	right = N - 1
	for left <= right {
...
func tambahMatakuliahMahasiswa(MH *daftarMahasiswa, MK *daftarMatakuliah, nMatakuliah, nMahasiswa int) {
	...
	if idx == -1 {
		...
		fmt.Scan(&nilai.Quiz)
		nilai.Total = (nilai.UTS + nilai.UAS + nilai.Quiz) / 3
...
func editMatakuliahMahasiswa(MH *daftarMahasiswa, MK *daftarMatakuliah, Nmahasiswa int, Nmatakuliah int) {
	var nim, kode, kode_baru string
	fmt.Print("Masukkan NIM mahasiswa: ")
	fmt.Scan(&nim)
	fmt.Print("Masukkan kode matakuliah yang ingin diedit: ")
	fmt.Scan(&kode)
	idxmhs := sequentialSearchMahasiswa(MH, Nmahasiswa, nim)
	if idxmhs == -1 {
		fmt.Println("Mahasiswa tidak ditemukan.")
		return
	}
	matkulIdx := sequentialSearchMatkulMahasiswa(MH, idxmhs, kode)
	if matkulIdx == -1 {
		fmt.Println("Matakuliah mahasiswa tidak ditemukan.")
		return
	}

	fmt.Print("Masukkan kode matakuliah baru: ")
	fmt.Scan(&kode_baru)
	idxmknew := binarySearchMatakuliah(MK, Nmatakuliah, kode_baru)
	if idxmknew == -1 {
		fmt.Println("Matakuliah baru tidak ditemukan di database.")
		return
	}
	MH[idxmhs].Matkul[matkulIdx] = (*MK)[idxmknew]
	MH[idxmhs].SKS -= (*MK)[matkulIdx].SKS
	MH[idxmhs].SKS += (*MK)[idxmknew].SKS
	MH[idxmhs].Transkrip[matkulIdx].Kode = (*MK)[idxmknew].Kode
	MH[idxmhs].Transkrip[matkulIdx].NamaMK = (*MK)[idxmknew].Nama

	fmt.Printf("Matakuliah %s berhasil diubah menjadi %s untuk mahasiswa %s\n", kode, kode_baru, MH[idxmhs].Nama)
}
